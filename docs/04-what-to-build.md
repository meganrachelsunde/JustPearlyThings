...
**Domain (resolved July 2026):** `justpearlythings.com` is held by a third party (unused), so the hub launches on **`hannahpearldavis.com`**, registered July 2026 on Namecheap. Page titles and content still target "JustPearlyThings" as the search term. Still worth doing: re-register the dead `theaudacitynetwork.store` and `audacitymerch.com` if available and redirect them to the live merch store, since hundreds of old episode descriptions still link to them; optionally WHOIS/backorder `justpearlythings.com` for the future.
